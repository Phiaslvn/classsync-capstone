...
/** Admin JSON/report endpoints live under ../../admin/ when served from views/admin or views/moderator. */
function isAdminOrModeratorDashboardView() {
  const p = window.location.pathname;
  return p.includes('/views/admin/') || p.includes('/views/moderator/');
}

function isModeratorDashboardView() {
  return window.location.pathname.includes('/views/moderator/');
}

// Dashboard Data Loading Functions
function loadDashboardData() {
  if (isModeratorDashboardView()) {
    return;
  }
  console.log('Loading dashboard data...');
  
  // Determine correct path based on current location
  const dashboardPath = isAdminOrModeratorDashboardView() 
  ...
  fetch(dashboardPath)
    ...
    .then(data => {
      ...
      if (data.success) {
        ...
        // Also load accounts data for the user management component
        loadAccounts();
      } else {
...
function loadProfileData() {
  ...
  // Determine correct path based on current location
  const profilePath = isAdminOrModeratorDashboardView() 
    ? '../../admin/reports/get_profile_data.php' 
...
function saveProfile() {
  ...
  const profileSavePath = isModeratorDashboardView() ? 'update_profile.php' : '../update_profile.php';
  fetch(profileSavePath, {
    method: 'POST',
    body: formData
  })
  .then(response => response.json())
...
function loadBasicStats() {
  // Determine correct path based on current location
  const statsPath = isAdminOrModeratorDashboardView() 
    ? '../../admin/reports/get_basic_stats.php' 
    : 'get_basic_stats.php';
...
function changeStep(direction) {
  ...
  if (adminCurrentStep === window.totalSteps) {
...
function addAccount() {
  ...
  .then(data => {
    if (data.success) {
      ...
      const isDuplicateError = data.name_exists || data.email_exists || 
        (data.message && (
      ...
      if (isDuplicateError && typeof Swal !== "undefined") {
        // Use styled duplicate modal
        let errorTitle = "Duplicate Account Detected";
        let errorMessage = '';
        let errorDetails = '';
        
        if (data.name_exists && data.duplicate_roles && data.duplicate_roles.length > 0) {
          const existingName = data.existing_name || '';
          const roleNames = data.duplicate_roles.map(r => r.role_name);
          const roleBadges = roleNames.map(name => `<span class="badge bg-danger">${name}</span>`).join(' ');
          
          errorMessage = `A user with this name already exists with the following role(s):`;
          errorDetails = `
            ...
            <div class="duplicate-alert-content">
              <div class="duplicate-user-info">
                ...
                <div class="user-email-display mt-2">
                  ...
                  <strong>${data.existing_email || ''}</strong>
                </div>
              </div>
              <div class="alert-warning-box mt-3">
                <i class="bi bi-exclamation-triangle-fill me-2"></i>
        ...
        Swal.fire({
          ...
          `,
          confirmButtonColor: "#800000",
          confirmButtonText: '<i class="bi bi-check-lg me-1"></i>Understood',
          customClass: {
  ...
  .catch(error => {
    ...
    if (typeof Swal !== "undefined") {
      Swal.fire({
        ...
        html: `<p>An error occurred while creating account.</p><p class="text-muted small mt-2">${error.message || 'Please check the browser console (F12) for details.'}</p>`,
        confirmButtonColor: "#dc3545",
        confirmButtonText: "OK",
      });
    } else {
...
function loadAccounts() {
  ...
  // Determine correct path based on current location
  const accountsPath = isAdminOrModeratorDashboardView() 
    ? '../../admin/management/get_accounts.php' 
    : 'get_accounts.php';
...
function performDeleteUser(accId, userName) {
  ...
  const deleteAccountPath = isModeratorDashboardView()
    ? '../../admin/management/delete_account.php'
    : 'delete_account.php';
  // Send delete request
...
function saveAccountEdits() {
  ...
  // Determine correct path based on current location
  const updatePath = isAdminOrModeratorDashboardView() 
    ? '../../admin/management/update_account.php' 
    : 'update_account.php';
...
function uploadProfilePicture(input) {
  if (input.files && input.files[0]) {
    ...
    const uploadProfilePath = isModeratorDashboardView()
      ? '../../shared/profile/upload_profile_picture.php'
      : '../upload_profile_picture.php';
    fetch(uploadProfilePath, {
      method: 'POST',
      body: formData
    })
    .then(response => {
    ...
    .then(data => {
      Swal.close();
      if (data.success) {
        // Update profile picture (add ../ prefix for admin dashboard)
        const profilePic = document.getElementById('profilePicture');
        if (profilePic) {
          profilePic.src = '../' + data.image_path + '?t=' + new Date().getTime();
        }
...
function removeProfilePicture() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      if (currentPath.includes('/views/') || currentPath.includes('/instructor/') || currentPath.includes('/admin/') || currentPath.includes('/moderator/')) {
        // From views or role-specific directories, go up to root then to shared
        removePath = '../../shared/profile/remove_profile_picture.php';
      } else if (currentPath.includes('/public/')) {
        // From public directory
        removePath = '../shared/profile/remove_profile_picture.php';
      } else {
        // From root or assets directory
        removePath = 'shared/profile/remove_profile_picture.php';
      }
...
function loadCourseData() {
  ...
  // Determine correct path based on current location
  const coursePath = isAdminOrModeratorDashboardView() 
...
// Curriculum Management Functions
// NOTE: The real implementation is in admin/assets/js/admin_dashboard.js
// Since admin_dashboard.js loads BEFORE this file (line 347 vs 953 in dashboard.php),
// this stub function would override the real one. We should NOT define it here.
// If you see "loadCurriculumData called - implementation should be in admin_dashboard.js",
// it means this stub is being called instead of the real function.
// 
// SOLUTION: Store reference to real function before this file loads, or remove this stub entirely.
// For now, we'll try to preserve the real function by storing it first.
if (typeof window._realLoadCurriculumData === 'undefined' && typeof loadCurriculumData !== 'undefined') {
  // Store the real function before we potentially override it
  window._realLoadCurriculumData = loadCurriculumData;
}

// Don't define the stub - let admin_dashboard.js's function be the only one
// If you need a fallback, check for window._realLoadCurriculumData instead
