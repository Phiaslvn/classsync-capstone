...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Get days as array (from checkboxes), use first day for editing single schedule
    $schd_days = isset($_POST['schd_day']) ? (is_array($_POST['schd_day']) ? $_POST['schd_day'] : [$_POST['schd_day']]) : [];
    if (empty($schd_days)) {
        $response['message'] = 'Please select at least one day.';
        echo json_encode($response);
        exit;
    }
    $schd_day = $schd_days[0]; // Use first selected day for editing
    $schd_start = $_POST['schd_start'];
    $schd_end = $_POST['schd_end'];
    $schd_status = $_POST['schd_status'];

    if (empty($schd_id)) {
        $response['message'] = 'Schedule ID is missing.';
        echo json_encode($response);
        exit;
    }
    ...
    try {
        $subj_stmt = $conn->prepare("SELECT dept_id, subj_lec, subj_lab FROM subject WHERE subj_id = ?");
        ...
        if ($subject) {
            ...
            // Exclude the current schedule being updated to calculate remaining hours correctly
            $total_sched_stmt = $conn->prepare("
                ...
                WHERE sy_id = ? 
                  ...
                  AND schd_id != ?
            ");
            $total_sched_stmt->bind_param("iiiii", $sy_id, $schd_term, $subj_id, $sec_id, $schd_id);
            ...
            // Calculate remaining hours (before adding the new schedule hours)
            $remaining_lec = max(0, $subj_lec - $total_scheduled_lec);
            $remaining_lab = max(0, $subj_lab - $total_scheduled_lab);
            
            // Check if there are remaining hours for the schedule type being updated
            ...
            // Check if updating this schedule would exceed the subject's required hours
            ...
            if ($subj_lec > 0 && $new_total_lec > $subj_lec) {
                ...
                $subj_code_stmt = $conn->prepare("SELECT subj_code, subj_desc FROM subject WHERE subj_id = ?");
                $subj_code_stmt->bind_param("i", $subj_id);
                $subj_code_stmt->execute();
                $subj_code_result = $subj_code_stmt->get_result();
                $subj_info = $subj_code_result->fetch_assoc();
                $subj_code_stmt->close();
                
                $subj_code = $subj_info['subj_code'] ?? 'Unknown';
                $subj_desc = $subj_info['subj_desc'] ?? '';
            ...
            if ($has_remaining_hours) {
                $other_instructor_stmt = $conn->prepare("
                    ...
                    LIMIT 1
                ");
                $other_instructor_stmt->bind_param("iiiiii", $sy_id, $schd_term, $subj_id, $sec_id, $inst_id, $schd_id);
        ...
        // 1. Check for Room Conflict (excluding the current schedule)
        ...
        if (!$allowsOverlap) {
            $conflict_stmt = $conn->prepare(
                ...
                WHERE s.sy_id = ? AND s.schd_term = ? AND s.schd_day = ? AND s.rm_id = ? AND s.schd_status = 'Active' AND s.schd_start < ? AND s.schd_end > ? AND s.schd_id != ?
                AND UPPER(TRIM(b.bd_desc)) NOT IN ('VIRTUAL', 'SCHOOL GROUND', 'SCHOOL GROUNDS')"
            );
            $conflict_stmt->bind_param("iisissi", $sy_id, $schd_term, $schd_day, $rm_id, $schd_end, $schd_start, $schd_id);
            $conflict_stmt->execute();
            $result = $conflict_stmt->get_result();
            if ($result->num_rows > 0) {
                $conflict = $result->fetch_assoc();
                $response['message'] = sprintf(
                    'Room Conflict: The selected room is already booked for %s (%s) with instructor %s from %s to %s.',
                    $conflict['subj_code'],
                    $conflict['sec_name'],
        ...
        // 2. Check for Instructor Conflict (excluding the current schedule)
        $conflict_stmt = $conn->prepare(
            ...
            JOIN section sec ON s.sec_id = sec.sec_id
            JOIN room r ON s.rm_id = r.rm_id
            WHERE s.sy_id = ? AND s.schd_term = ? AND s.schd_day = ? AND s.inst_id = ? AND s.schd_status = 'Active' AND s.schd_start < ? AND s.schd_end > ? AND s.schd_id != ?"
        );
        $conflict_stmt->bind_param("iisissi", $sy_id, $schd_term, $schd_day, $inst_id, $schd_end, $schd_start, $schd_id);
        $conflict_stmt->execute();
        $result = $conflict_stmt->get_result();
        if ($result->num_rows > 0) {
            $conflict = $result->fetch_assoc();
            $response['message'] = sprintf(
                'Instructor Conflict: The selected instructor is already scheduled for %s (%s) in room %s from %s to %s.',
                $conflict['subj_code'],
                $conflict['sec_name'],
                $conflict['rm_name'],
                $conflict['start_time'],
        ...
        // 3. Check for Section Conflict (excluding the current schedule)
        ...
        $isVirtualRoom = false;
        if ($roomData && strcasecmp(trim($roomData['bd_desc']), 'Virtual') === 0) {
            $isVirtualRoom = true;
        }
        
        // Only check for section conflicts if the new schedule is NOT in a virtual room
        if (!$isVirtualRoom) {
            $conflict_stmt = $conn->prepare(
                ...
                JOIN instructor i ON s.inst_id = i.inst_id
                JOIN building b ON r.bd_id = b.bd_id
                WHERE s.sy_id = ? AND s.schd_term = ? AND s.schd_day = ? AND s.sec_id = ? AND s.schd_status = 'Active' AND s.schd_start < ? AND s.schd_end > ? AND s.schd_id != ?
                AND UPPER(TRIM(b.bd_desc)) NOT IN ('VIRTUAL', 'SCHOOL GROUND', 'SCHOOL GROUNDS')"
            );
            $conflict_stmt->bind_param("iisissi", $sy_id, $schd_term, $schd_day, $sec_id, $schd_end, $schd_start, $schd_id);
            $conflict_stmt->execute();
            $result = $conflict_stmt->get_result();
            if ($result->num_rows > 0) {
                $conflict = $result->fetch_assoc();
                $response['message'] = sprintf(
                    'Section Conflict: The selected section is already scheduled for %s with %s in room %s from %s to %s.',
        ...
        // After req_date has passed, fixed schedules can be added/updated
        ...
        if ($roomRequestResult->num_rows > 0) {
            ...
            while ($request = $roomRequestResult->fetch_assoc()) {
                ...
                $requestStart = $request['schd_start'];
                $requestEnd = $request['schd_end'];
            ...
            if ($userDeptId !== $requesterDeptId && $requesterDeptId > 0) {
                ...
                $response['message'] = sprintf(
                    ...
                    'Fixed schedules cannot be updated while there is an active room request.<br>' .
                    'Please wait until after the request date (%s) to update the fixed schedule.',
            ...
            if ($userDeptId === $requesterDeptId) {
                foreach ($allowedTimeRanges as $requestDay => $timeRanges) {
                    // Check if the schedule day matches the requested day
                    if ($schd_day !== $requestDay) {
                        $conflictFound = true;
                        $conflictDetails[] = [
                            'day' => $requestDay,
                            'time' => $timeRanges[0]['start_formatted'] . ' - ' . $timeRanges[0]['end_formatted'],
                            'requester' => $timeRanges[0]['requester_dept']
                        ];
                    } else {
                        ...
                        foreach ($timeRanges as $timeRange) {
                            ...
                            if ($scheduleStart < $requestStartTime || $scheduleEnd > $requestEndTime) {
                                ...
                                $conflictDetails[] = [
                                    'day' => $requestDay,
                                    'time' => $timeRange['start_formatted'] . ' - ' . $timeRange['end_formatted'],
                                    'requester' => $timeRange['requester_dept'],
                                    'schedule_time' => date('h:i A', strtotime($schd_start)) . ' - ' . date('h:i A', strtotime($schd_end))
                                ];
                            }
                        }
                    }
        ...
        $stmt = $conn->prepare("UPDATE schedule SET sy_id=?, program_id=?, year_level=?, dept_id=?, subj_id=?, sec_id=?, inst_id=?, rm_id=?, schd_type=?, schd_term=?, schd_day=?, schd_start=?, schd_end=?, schd_min=?, schd_status=?, is_overtime=? WHERE schd_id=?");
        $stmt->bind_param("iiiiiiiisssssissi", $sy_id, $program_id, $year_level, $dept_id, $subj_id, $sec_id, $inst_id, $rm_id, $schd_type, $schd_term, $schd_day, $schd_start, $schd_end, $schd_min, $schd_status, $is_overtime, $schd_id);
