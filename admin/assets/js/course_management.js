...
function populateModalDropdowns() {
  console.log('populateModalDropdowns called');
  // Populate department dropdowns in add modals
  const deptDropdowns = ['program_dept_id', 'curr_dept_id', 'subj_dept_id'];
  deptDropdowns.forEach(dropdownId => {
    const dropdown = document.getElementById(dropdownId);
    console.log(`Looking for dropdown: ${dropdownId}`, dropdown);
    if (dropdown) {
      dropdown.innerHTML = '<option value="">Select Department</option>';
      if (__COURSE_CACHE__.departments && __COURSE_CACHE__.departments.length > 0) {
        __COURSE_CACHE__.departments.forEach(dept => {
          const option = document.createElement('option');
          option.value = dept.dept_id;
          option.textContent = `${dept.dept_name} (${dept.college_name})`;
        ...
        console.log(`Populated ${__COURSE_CACHE__.departments.length} departments in ${dropdownId}`);
      } else {
        console.log('No departments in cache');
      }
    }
  });
  
  // Populate program dropdowns in add modals
  const programDropdowns = ['subj_program_id'];
...
// Function to load programs when department is selected for subject form
function loadProgramsForSubject(deptId) {
  const programDropdown = document.getElementById('subj_program_id');
  ...
  const departmentPrograms = __COURSE_CACHE__.programs.filter(program => program.dept_id == deptId);
    ...
    row.innerHTML = `
      ...
      <td>
        <div class="d-flex gap-1 justify-content-center">
          <button class="btn btn-md btn-outline-primary" onclick="editProgram(${program.program_id})" title="Edit Program">
            <i class="bi bi-pencil"></i>
          </button>
          <button class="btn btn-md btn-outline-danger" onclick="deleteProgram(${program.program_id})" title="Delete Program">
...
function saveSubject() {
  const form = document.getElementById('addSubjectFormCourse');
  const formData = new FormData(form);
  const btn = event.target;
  const original = btn.innerHTML;
  
  // Validate required fields
  const requiredFields = ['subj_code', 'subj_desc', 'subj_lec', 'subj_lab', 'subj_unit', 'subj_min', 'subj_lvl', 'subj_term', 'subj_category', 'dept_id', 'program_id', 'curr_id'];
...
// Test function to manually load course data (call from browser console)
window.testLoadCourseData = function() {
  console.log('Manual test: loading course data...');
  loadCourseData();
};
